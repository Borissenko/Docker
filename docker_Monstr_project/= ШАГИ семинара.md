...
а) Создаем глобальную переменную - адрес обращения сервисов к api-сервису.
...
services:
  api:
    environment:
      - AUTH_API_URL=http://auth:3002/api   //собственно значение переменной - префикс у адреса auth-сервиса в docker-сети.  
...
б) Прописываем в api/src/index.js
...
Более того, сетью my-network далее можно пользоваться by сервисами из других dockr-проектов.
...
#20 (урок 20) Проксирование фронтенда.
...
Б. Вариант-best, через Nginx.
URL для запроса будет просто axios("/api/post"),
...
что бы nginx стартовал только полле них.
